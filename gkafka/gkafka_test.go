...
func createExtractor(t *testing.T, ctx context.Context, spec *entity.Spec, config *Config, pf gki.ProducerFactory) entity.Extractor {
...
// concurrent stream instances. All events are deemed unretryable (from the Void sink config)
// and is thus put to the DLQ, matching with number of sent events.
func TestDLQE2E(t *testing.T) {
	...
	ef := NewExtractorFactory(kConfig, consumerFactory, dlqProducerFactory)
...
func (mcf *MockConsumerFactory) NewConsumer(conf *kafka.ConfigMap) (gki.Consumer, error) {
	return &MockConsumer{conf: conf, eventsToSimulate: mcf.eventsToSimulate}, nil
}

func (mcf *MockConsumerFactory) NewAdminClientFromConsumer(c gki.Consumer) (gki.AdminClient, error) {
	return &MockAdminClient{}, nil
}
